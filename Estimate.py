...
def create_variable(data, par, print_aux_reg = False, gender = "w", do_estimate_wage = "est_omega", print_reg = False, shadow_value_simple = 1, do_control_love = False, part_earning_simple = 1, control_cons = 1 , wealth_love = 1, do_true_barg = False, control_select = False):
    data = data.sort_values(by =['idx','t'])
    ...
    data  = Heckman_selection2(data, gender = gender, do_estimate_wage = do_estimate_wage, print_reg = print_reg, shadow_value_simple = shadow_value_simple , do_control_love = do_control_love , part_earning_simple =  part_earning_simple, control_cons = control_cons , wealth_love = wealth_love , do_true_barg =  do_true_barg, control_select = control_select )
...
def Heckman_selection(data,par, print_reg = False):
    ...
    data['select_IMR_temp'] = selection_model.predict(x,linear = True)
    #data['select_IMR'] = selection_model.pdf(data['select_IMR'])/selection_model.cdf(data['select_IMR'])
    data['select_IMR_temp'] = norm.pdf(data['select_IMR_temp'])/norm.cdf(data['select_IMR_temp'])
    
    
    return data


def Heckman_selection2(data, gender = "w", do_estimate_wage = "est_omega", print_reg = False, shadow_value_simple = 1, do_control_love = False, part_earning_simple = 1, control_cons = 1 , wealth_love = 1, do_true_barg = False, control_select = False):

    _, x = define_x(data, gender = gender, do_estimate_wage = do_estimate_wage, print_reg = print_reg, shadow_value_simple = shadow_value_simple , do_control_love = do_control_love , part_earning_simple =  part_earning_simple, control_cons = control_cons , wealth_love = wealth_love , do_true_barg =  do_true_barg, control_select = control_select, two_step = False )
   
    y = data['couple'] 
    x1 = data[['delta_Vw_single','delta_Vm_single','delta_Vw_single_l','delta_Vm_single_l','delta_Vw_single_l2','delta_Vm_single_l2']]
    
    x1 = pd.concat([x1, x ], axis=1 )
    
    x1 = sm.add_constant(x1)
    selection_model = Probit(y, x1).fit(disp = False)
    if print_reg:
        print('selection model')
        print(selection_model.summary())

    data['select_IMR'] = selection_model.predict(x1,linear = True)
...
def define_x(data, gender = "w", do_estimate_wage = "est_omega", print_reg = False, shadow_value_simple = 1, do_control_love = False, part_earning_simple = 1, control_cons = 1 , wealth_love = 1, do_true_barg = False, control_select = False, two_step = True ):
    ...
    df = df.drop(columns = ['delta_Vw_single','delta_Vm_single','delta_Vw_single_l','delta_Vm_single_l','delta_Vw_single_l2','delta_Vm_single_l2'])
    ...
    y_x = pd.concat([y,x, ID], axis=1)

    return y_x, x


def main_est2(data, gender = "w", do_estimate_wage = "est_omega", print_reg = False, shadow_value_simple = 1, do_control_love = False, part_earning_simple = 1, control_cons = 1 , wealth_love = 1, do_true_barg = False, control_select = False):
    
    y_x, _  = define_x(data, gender = gender, do_estimate_wage = do_estimate_wage, print_reg = print_reg, shadow_value_simple = shadow_value_simple , do_control_love = do_control_love , part_earning_simple =  part_earning_simple, control_cons = control_cons , wealth_love = wealth_love , do_true_barg =  do_true_barg, control_select = control_select )
    ...
    x = y_x.drop(columns = [f'y_{gender}','idx'])
    #x = x.T.drop_duplicates().T #drop duplicates
    x = sm.add_constant(x)  
    #result = sm.OLS(y,x).fit() 
    result = sm.OLS(y,x).fit().get_robustcov_results(cov_type = 'cluster', groups = y_x['idx'])
    N = result.nobs
    #SAVE WALD TEST
    if do_true_barg:
    ...
    return  data, Wald_FC, Wald_NC, Wald_FC_noW, Wald_NC_noW, coef_dict
...
def main_est(data, gender = "w", do_estimate_wage = "est_omega", print_reg = False, shadow_value_simple = 1, do_control_love = False, part_earning_simple = 1, control_cons = 1 , wealth_love = 1, do_true_barg = False, control_select = False):
    ...
    x = pd.concat([df,  X_t , Shadow_value, inc_share, cons, Wealth_and_Love ], axis=1 )
    if control_select:
        IMR = Heckman_selection2(data_regress,x,print_reg = print_reg)
        x = pd.condat(x, IMR)
    #x = x.T.drop_duplicates().T #drop duplicates
    x = sm.add_constant(x)  
    #result = sm.OLS(y,x).fit() 
